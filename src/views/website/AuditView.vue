<script setup>
    ...
    const fetchAudits = async () => {
        ...
        error.value = null
        try {
            const response = await apiService.get('/v1/audit-reports')
            const list = response?.data || response || []
            audits.value = Array.isArray(list) ? list : []
            const urls = audits.value.map((r) => r?.pdf_file).filter(Boolean)
            await Promise.all(urls.map((u) => ensureFileSize(u)))
        } catch (e) {
            error.value = e.message || 'Failed to load audit reports'
            audits.value = []
        } finally {
            loading.value = false
        }
    ...
    const langOrder = ['ru', 'en', 'tk']
    const getLangFiles = (pdfFiles) => {
        const out = []
        langOrder.forEach((code) => {
            const url = pdfFiles?.[code]
            if (url) out.push({ code, url })
        })
        return out
    }

    const ensureFileSize = async (url) => {
        if (!url || sizeMap.value[url]) return
        try {
    ...
    const toProxiedUrl = (u) => {
        if (!import.meta.env.DEV || !u) return u
        try {
            const url = new URL(u)
            if (url.pathname.startsWith('/storage')) {
                return `/storage${url.pathname.replace('/storage', '')}`
            }
            // direct path like http://host/storage/...
            const match = url.pathname.match(/\/storage\/.*$/)
            if (match) return `/storage${url.pathname.split('/storage')[1] ? '/' + url.pathname.split('/storage')[1] : ''}`
            return u
        } catch {
            return u
        }
