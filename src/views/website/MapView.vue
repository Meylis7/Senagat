<script setup>
    ...
    const DEFAULT_CENTER = [37.90, 58.37];
    const MAP_ZOOM = 14;
...
<template>
    <section class="block fixed top-0 left-0 w-full h-screen">
        ...
        <div class="flex items-start flex-col lg:flex-row gap-4 overflow-auto fixed top-0 left-0 pt-[110px] lg:pt-[200px] w-screen h-screen z-[35] p-4 pointer-events-none"
            :class="{ 'bg-[#EEF2ED]': isBranchesOpen }">
            <div
                class="bg-[#F7F8F699] w-full md:w-auto sm:min-w-[310px] p-2 sm:p-5 rounded-lg sm:rounded-[20px] border-solid border-1 border-white pointer-events-auto">
                <button type="button" @click="toggleBranchesOpen"
                    class="block text-sm sm:text-[17px] font-bold text-white bg-[#2C702C] rounded-[10px] p-2 sm:px-5 sm:py-[14px] mb-5 w-full">
            ...
            <div v-show="isBranchesOpen" class="w-full pointer-events-auto">
                <div class="grid grid-cols-12 gap-4 pb-[60px] w-full">
                    <div v-for="(branch, idx) in filteredLocations" :key="branch.id || idx"
                        class="flex flex-col col-span-12 md:col-span-6 lg:col-span-4 gap-5 bg-[#F7F8F6] rounded-[20px] p-5">
                        <div class="block">
                        ...
                        <div class="grid gap-5 mt-auto">
                            ...
                            <div class="block" v-if="branch.help_desk_number">
                                <div class="flex">
                                    ...
                                    <a :href="'tel:' + branch.help_desk_number"
                                        class="text-[#2C702C] text-sm font-Gilroy">
